...
module closure()
{
    ...
    difference()
    {
        ...
        difference()
        {
            ...
            translate([g_x-_fc_xo,wt,-eps])
                cube([_fc_xo,g_d-eps,g_z-wt]);
            // back reinforcement
            translate([0,g_y-g_d,-eps])
                cube([_bc_xo,g_d-eps,g_z-wt]);
            translate([g_x-_bc_xo,g_y-g_d,-eps])
                cube([_bc_xo,g_d-eps,g_z-wt]);
        }
        ...
        translate([g_x/2-n_x/2,g_y+s_y,0])
        {
            ...
            translate([n_x/2-f_x/2,-f_ye+eps,n_z-f_d+eps])
                cube([f_x,n_y+2*eps+f_ye,f_d]);
            ...
            translate([-eps,n_y-sc_y2+eps,-eps])
                cube([(n_x-f_x)/2+2*eps,sc_y2,sc_ch+2*eps]);
    ...
    // upper screw shafts
    translate([ g_x/2-n_x/2+nhu_xo+nhu_l,
                g_y+s_y+nhu_yo,
                -nhu_rho-eps])
    {
        difference()
        {
            hull()
            {
                cylinder(h=n_z-wt+nhu_rho,d=nhu_rd+2*wt);
                // right upper hole
                translate([0,0,n_z-nhu_rh])
                    cylinder(h=nhu_rh-wt+nhu_rho,d=nhu_rD+2*wt);
            }
